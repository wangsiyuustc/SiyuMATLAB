function [xfit, negLLbest] = MLEfit_choicecurve(choice, dR, additionalX, isbias, islapserate, X0struct, LBstruct, UBstruct, nX0, priorFlag, isdisplay)
% [xfit, negLLbest] = MLEfit_choicecurve(choice, dR, additionalX, isbias, islapserate, X0struct, LBstruct, UBstruct, nX0, priorFlag, isdisplay)
    % need to implement priorFlag
    % noise may need to be multiplied by sqrt(2)
    if ~exist('nX0'); % number of initial points
        nX0 = 1;
    end
    if exist('isdisplay') && strcmp(isdisplay, 'on')
    ...
    for repi = 1:nX0
        ...
        if  isempty(islapserate) || all(islapserate == 0)
            if isbias
                problems.objective = @(x)-logLL_choicecurve(choice, dR, x(2), additionalX, x(1), x(3:end), 0, 0);
            else
        ...
        elseif all(islapserate == [1 0])
            if isbias
                problems.objective = @(x)-logLL_choicecurve(choice, dR, x(2), additionalX, x(1), x(3:end-1), x(end), 0);
            else
                problems.objective = @(x)-logLL_choicecurve(choice, dR, 0, additionalX, x(1), x(2:end-1), x(end), 0);
            end
            [xfit_raw, negLL] = fmincon(problems);
            if negLL < negLLbest
                negLLbest = negLL;
        ...
        elseif all(islapserate == [0 1])
            if isbias
                problems.objective = @(x)-logLL_choicecurve(choice, dR, x(2), additionalX, x(1), x(3:end-1), 0, x(end));
            else
                problems.objective = @(x)-logLL_choicecurve(choice, dR, 0, additionalX, x(1), x(2:end-1), 0, x(end));
            end
            [xfit_raw, negLL] = fmincon(problems);
            if negLL < negLLbest
                negLLbest = negLL;
            ...
            if isbias
                problems.objective = @(x)-logLL_choicecurve(choice, dR, x(2), additionalX, x(1), x(3:end-2), x(end-1), x(end));
            else
                problems.objective = @(x)-logLL_choicecurve(choice, dR, 0, additionalX, x(1), x(2:end-2), x(end-1), x(end));
            end
            [xfit_raw, negLL] = fmincon(problems);
            if negLL < negLLbest
                negLLbest = negLL;
...
function [LL] = logLL_choicecurve(choice, dR, bias, additionalX, noise, w_additionalX, lapse1, lapse2)
